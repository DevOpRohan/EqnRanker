...
vectorStore = VectorStore(storage_file="persistence/vector_store.pkl")
...
# preprocessed_contents = preprocessor.preprocess_text_file(file_path="data/sample.txt", delimiter="====")
#
# # save the preprocessed contents into text file, each element will be seperated by "\n====NEW_QUESTION====\n"
# with open("data/processedsample.txt", "w") as file:
...
preprocessed_query = asyncio.run(preprocessor.preprocessContent(query))
print(f"\nPreprocessed Query:\n {preprocessed_query} \n")
